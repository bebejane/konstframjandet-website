...
export default function StructuredContent({ record, content }: Props) {
	...
	return (
		<StructuredText
			...
			renderBlock={({ record: block }) => {
				return <Block data={block} record={record} />;
			}}
				...
				renderNodeRule(
					...
					({ adapter: { renderNode }, node, children, key, ancestors }) => {
						...
						if (isRoot(ancestors[0]) && node.children[0].value?.startsWith('\n')) {
							let index = 0;
							while (
								index < node.children[0].value.length &&
								node.children[0].value[index] === '\n'
							)
								index++;
						...
						children = children.filter(
							(c) => !(c.props.children.length === 1 && !c.props.children[0]),
						);
